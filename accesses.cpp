/******************************************************************************
# Author:           Carlos Soto
# Assignment:       Assignment 4 Accesses
# Date:             03/03/2024
# Description:      This program Sorts users by System and outputs the system with
#                   all the people that accessed that system, the amount of times
#                   each pearson accessed it and the percentage of accesses.
#
# Input:            Program ask for two txt files and the max number of people
# Output:           System: Power
                        Top: Carlos Soto
                        All:
                          Carlos Soto 1 (100%)
# Sources:          Assignment 4 specification
#*****************************************************************************/
...
int main(int argc, char *argv[])
{
  ...
  {
    cout << "Usage: " << argv[0]
         << " <access log file> <systems file> <num staff>" << endl;
    return (0);
  }
  ...
  // insert these accesses into our arrays of data. The first line of the
  // logfile tells us how many entries are in that logfile. Each line of the
  // logfile contains a system name and the person accessing it. These two
  // values are separated by a comma. This code here will read the system name
  // and person name then it will loop over all the system logs (in the array
  // created above) and give each system log a change to enter that data. A
  // system log will only enter the data if it is the right one. That is, the
  // system name matches the name in that log.
  ifstream in(logfile);
  int numEntries;
  in >> numEntries;
  in.get();
  for (int i = 0; i < numEntries; i++)
  {
    ...
    // one. The Syslog object will check to see if it matches the given
  ...
  //  allLogs[0]->copySysentry();

  // Overloaded assignment operator test
  //  allLogs[1]->assignmentoperator();
